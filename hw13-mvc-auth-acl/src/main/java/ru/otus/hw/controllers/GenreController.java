...
public class GenreController extends AbstractAccessController {
        ...
        return accessValidate(
            () -> {
                if (genre.id() != null) {
                    ...
                    var newGenre = genreService.create(genre);
                    return "redirect:/genres/%d".formatted(newGenre.id());
                }
            },
            () -> {
    ...
    private String deleteGenre(@PathVariable long id, Model model) {
        return accessValidate(
            () -> {
